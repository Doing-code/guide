...
最少活跃调用树，相同活跃数的随机，活跃数指调用前后计数差。

使得慢的提供者收到更少请求，因为越慢的提供者的调用前后计数差会越大。
...
> URL：protocol://username:password@host:port/path?key=value&key=value
> 
...
### 启动解析、加载配置信息
解析配置的入口 `org.apache.dubbo.config.spring.schema.DubboNamespaceHandler#parse`，上层由`org.springframework.beans.factory.xml.BeanDefinitionParserDelegate#parseCustomElement(org.w3c.dom.Element, org.springframework.beans.factory.config.BeanDefinition)`调用。
```java
...
org.apache.dubbo.config.spring.context.DubboBootstrapApplicationListener#onApplicationContextEvent
    org.apache.dubbo.config.spring.context.DubboBootstrapApplicationListener#onContextRefreshedEvent
        org.apache.dubbo.config.bootstrap.DubboBootstrap#start
            org.apache.dubbo.config.bootstrap.DubboBootstrap#exportServices 【调用获取到所有的ServiceBean对象并挨个export】
...
配置检查完毕后，紧接着要做的事情是根据配置，以及其他一些信息组装 URL。URL 是 Dubbo 配置的载体，通过 URL 可让 Dubbo 的各种配置在各个模块之间传递。

该方法主要逻辑：根据 host、post等信息，构建服务对外提供的 URL。还提供服务暴露功能，本地服务和远程服务。
```java
...
public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
    ...
    // export service.
    // 获取服务标识（服务坐标）。由服务组名+服务名+服务版本号+端口组成。比如：demoGroup/com.alibaba.dubbo.demo.DemoService:1.0.1:20880
...
private ProtocolServer createServer(URL url) {
  ...
  // 通过 SPI 检测是否存在 server 参数对应的 Transporter 扩展，不存在则抛异常
  ...
  // 通过 SPI 检测是否存在 client 参数对应的 Transporter 扩展，不存在则抛异常
...
/*
  1. 会创建一个 NettyServer，会调用 NettyServer 的父类构造 AbstractServer 进行配置获取。如 bindIp、bindPort，构造 localAddress，最大可接受连接数等
...
```
程序使用的是 Zookeeper 作为注册中心，所以使用的是`ZookeeperRegistry`，但是`ZookeeperRegistry`中没有重写`register()`方法，所以调用的是父类中的方法（`org.apache.dubbo.registry.support.FailbackRegistry#register`）。
