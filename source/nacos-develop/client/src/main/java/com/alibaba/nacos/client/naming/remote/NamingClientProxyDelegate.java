...
public class NamingClientProxyDelegate implements NamingClientProxy {
    ...
    public ServiceInfo subscribe(String serviceName, String groupName, String clusters) throws NacosException {
        ...
        // serviceNameWithGroup = groupName@@serviceName
        String serviceNameWithGroup = NamingUtils.getGroupedName(serviceName, groupName);

        // serviceKey = serviceNameWithGroup@@clusters || serviceNameWithGroup
        String serviceKey = ServiceInfo.getKey(serviceNameWithGroup, clusters);

        // 定时调度 服务信息更新服务任务 UpdateTask
        // 客户端周期性的从服务端 pull 服务信息，【而服务端每当有服务注册时，也会主动 push 服务信息给订阅者】；Nacos 采用 pull + push 结合
        // 而 nacos 采用长轮询 pull
        serviceInfoUpdateService.scheduleUpdateIfAbsent(serviceName, groupName, clusters);

        // 获取服务信息
        // 若本地缓存中没有服务实例信息，执行订阅，获取返回的服务信息
        ServiceInfo result = serviceInfoHolder.getServiceInfoMap().get(serviceKey);
        if (null == result || !isSubscribed(serviceName, groupName, clusters)) {
            // 若服务实例信息为null，基于gRPC协议进行订阅逻辑处理，并获取Nacos服务端返回的服务信息
            result = grpcClientProxy.subscribe(serviceName, groupName, clusters);
        }

        // 客户端本地缓存从Nacos服务端获取的服务实例信息
        // 首次会将服务实例缓存在 serviceInfoMap 中
