...
public class EphemeralClientOperationServiceImpl implements ClientOperationService {
    ...
    public void registerInstance(Service service, Instance instance, String clientId) throws NacosException {
        ...
        // 一个 instance 即一个实例，也就是一个 Client，而 ClientManager 是所有实例的管理器
        ...
        // 将 Instance 储存到 Client
        // 发布ClientChangeEvent事件，用来向集群中其他节点增量同步新增的 Client 信息
        client.addServiceInstance(singleton, instanceInfo);
        client.setLastUpdatedTime();
        client.recalculateRevision();

        // 建立 service 和 clientId 的关系，客户端注册事件，ClientServiceIndexesManager 监听
        // 服务端在配置数据有变动时，通过建立的长连接立即将数据推送（push）给客户端‌（服务消费方）
