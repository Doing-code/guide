...
public class InstanceOperatorClientImpl implements InstanceOperator {
    ...
    /**
     * 这个 registerInstance 方法，实际上是创建了一个 client
     *   如果这个 client 是第一次注册上来，则创建，如果不是则不做处理，
     *   如 clientId 192.168.0.6:8080#true，这里的true指短暂的客户端，第一次注册的话，则会生成一个新的client
     *
     * 服务提供者向 Nacos服务端 注册服务
     *  简单来讲，服务注册的目的就是客户端将自己的ip端口等信息上报给 Nacos 服务端
     *
     * 当一个服务发生上下线，Nacos如何知道要推送给哪些客户端？
     *  假设订阅 A 服务，当A服务有新的实例启动时，就会推送给服务消费者（客户端），
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        ...
        // 判断实例是否为临时节点，默认为 true
        boolean ephemeral = instance.isEphemeral();

        // 获取客户端id, 一个客户端gRPC长连接对应一个Client，每个Client有自己唯一的id（clientId）
        String clientId = IpPortBasedClient.getClientId(instance.toInetAddr(), ephemeral);

        // 通过客户端Id获取连接
        // Nacos在2.0版本之后新增了Client模型，每个Client都有自身唯一的ClientId
        // 一个gRPC长连接对应一个Client。Client负责管理一个客户端的服务实例注册Publish和服务订阅Subscribe
