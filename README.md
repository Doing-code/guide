...
抽奖系统为什么自研路由组件？
    1. 公司内部技术栈建设时，类似mycat不能满足，ss还没有完全稳定。不过目前内部也有部分项目使用ss
    ...
    ShardingSphere 最早期版本存在的一些问题包括：
        1. 早期版本中缺乏完善的文档和示例，使用时需要自行验证demo，且分库分表配置比较麻烦。
        2. 如我们需要按照业务特性，自定义分片键、一个字段分库、一个字段分表、分布式事务等都没有完善。
        3. 压测时的性能问题，在高并发场景下可能出现性能瓶颈等。压测效果不太好。
        4. 早期版本中使用起来比较复杂，需要进行大量的配置和编码工作，内部需要一个更简单的路由组件，让研发尽可能快速使用
